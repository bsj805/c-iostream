...
public:
	...
	/*void lockstackinsert(K searchKey,V newValue)//stack+ 작은거는 동시insert
    {
        skiplist_node<K,V,MAXLEVEL>* update[MAXLEVEL];
        NodeType* currNode = m_pHeader;
		//checklow m_minKey
		...
		skiplistmutex.lock();
		if(checklow!=m_minKey)
		{
			while(checklow!=m_minKey && searchKey>= checklow) usleep(1);
		}
		else
		{
			checklow=searchKey;
        ...
        for(int level=max_curr_level; level >=1; level--) {
			...
			if(level==max_curr_level && (checklow > currNode->key[0] || checklow==m_minKey)) checklow=currNode->key[0];
            update[level] = currNode;
			loopmutex.unlock();
        }
	...
	void finalstackinsert(K searchKey,V newValue)//일단임시제출용
    {
        skiplist_node<K,V,MAXLEVEL>* update[MAXLEVEL];
        NodeType* currNode = m_pHeader;
		//int locking;
		skiplist_node<K,V,MAXLEVEL>* lockednode;
		do
		{
		//std::unique_lock<std::mutex> skiplk(skiplistmutex);
		//do{splitcv.wait(skiplk,[&]{return checklow == m_minKey || searchKey < checklow;});
		//}while(checklow==m_minKey || searchKey < checklow);
		//loopmutex.lock();
		//skiplistmutex.lock(); conditional variable splitcv
        for(int level=max_curr_level; level >=1; level--) {
            while ( currNode->forwards[level]->key[0] <= searchKey ) {
                currNode = currNode->forwards[level];
            }	
		...
		lockednode=currNode;
	if( currNode->cnt < NPAIRS){
		while(currNode->forwards[1]->key[0] <= searchKey)
		{currNode=currNode->forwards[1];}
		if(lockednode!=currNode)
		{
			//cout<<lockednode->key[0]<<"hi?"<<currNode->key[0]<<endl; 이 새노드는 이미 또 cnt 가npair만큼일수도
			(lockednode->nodelock).unlock();
			continue;
			//(currNode->nodelock).lock();
		}
		//cout<<"start insert "<<searchKey <<endl;
	    //  insert
	    //currNode->insert(searchKey, newValue);
		if(currNode->cnt==0)
		{
			...
			//(lockednode->nodelock).unlock();
			(currNode->nodelock).unlock();
			currNode->value[(currNode->cnt)-1]=newValue;
		...
		else if(currNode->key[0] > searchKey) //searchKey가 key[0]되야
		{
			...
			//(lockednode->nodelock).unlock();
			//skiplistmutex.unlock();
			currNode->value[(currNode->cnt)-1]=currNode->value[0];
			currNode->value[0]=newValue;
		...
		if(currNode->cnt<NPAIRS)
		{
			...
			skiplistmutex.unlock();
			if(currNode!=lockednode)
			{
				(lockednode->nodelock).unlock();
				(currNode->nodelock).lock();
			}
			
			if(currNode->key[0]>searchKey)
			{
				currNode->key[currNode->cnt]=currNode->key[0];
				(currNode->cnt)++;
				currNode->key[0]=searchKey;
		...
		{
		for(int i=1;i<currNode->cnt;i++)
		{
				int temp=currNode->key[i];
				int temp2=currNode->value[i];
				int j;
				for(j=i;j>0 && currNode->key[j-1] > temp; j--)
				{
					currNode->key[j]=currNode->key[j-1];
					currNode->value[j]=currNode->value[j-1];
				}
				currNode->key[j]=temp;
				currNode->value[j]=temp2;
	...
	void stackinsert(K searchKey,V newValue)
    {
		...
		std::unique_lock<std::mutex> skiplk(skiplistmutex);
		splitcv.wait(skiplk,[&]{return checklow == m_minKey || searchKey < checklow;});
        ...
        for(int level=max_curr_level; level >=1; level--) {
			...
			if(level==max_curr_level && (checklow > currNode->key[0] || checklow == m_minKey))		{	 		
					checklow= currNode->key[0];
					skiplistmutex.unlock();
			}
			/if(level==max_curr_level)
			{
				(currNode->nodelock).lock();//이게 풀렸는데, 다음노드를 가리킬수도 있어.
				lockednode=currNode;
				while(currNode->forwards[level]->key[0] <= searchKey)
				{
					currNode = currNode->forwards[level];
	...
	if( currNode->cnt < NPAIRS){
		...
		//cout<<"start split "<<searchKey<<endl;
		newinsort(currNode->key,currNode->cnt);
		newinsort(currNode->value,currNode->cnt);
	    int newlevel = randomLevel();
	    if ( newlevel > max_curr_level ) {
