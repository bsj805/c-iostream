...
///////////////////////////////////////semaphore
			...
			quelock.unlock();
/////////////////////////////////////////////
			//cout<<num<<"["<<count<<"]inside thread "<<dec<<" "<<den<<endl;
			if(dec=='i')
			{
				//cout<<num<<"I insert"<<den<<endl;
				//skiplistlock.lock();
				//skiparr->lockstackinsert(den,den);//동시에insert가능
				skiparr->finalstackinsert(den,den);
				//skiparr->stackinsert(den,den);
				//skiparr->lockinsert(den,den);
				//skiparr->newinsert(den,den);
				//skiplistlock.unlock();
			}
			else if(dec=='q')
			{
	...
	*/
	...
	while(fscanf(fin, "%c %ld\n", &action, &num) > 0) {
		...
		if(action=='p')
		{
			...
			std::unique_lock<std::mutex> prlk(quelock);
			printcv.wait(prlk,[&]{return one->isempty(); });
			//while(!(one->isempty())){			usleep(0.5);}
			//quelock.lock();//아무도 새로 일을 못받아.
			//printlock.lock();//단순히 semaphore보호 이거하면 데드락. 아무도 세마포안풀어
			//여기서sort하면안되는 이유: 지금 insert중일수도.
